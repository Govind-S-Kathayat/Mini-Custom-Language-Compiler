...
function generate(node, language) {
    switch (node.type) {
        case 'program':
            const bodyCode = node.body.map(stmt => generate(stmt, language)).join('\n');
        ...
        case 'declaration':
            if (language === 'js') return `let ${node.name} = ${node.value};`;
            if (['cpp', 'java', 'c'].includes(language)) return `int ${node.name} = ${node.value};`;
            if (language === 'python') return `${node.name} = ${node.value}`;
        ...
        case 'print':
            if (language === 'js') return node.isString ? `console.log("${node.value}");` : `console.log(${node.value});`;
            if (language === 'cpp') return node.isString ? `std::cout << "${node.value}" << std::endl;` : `std::cout << ${node.value} << std::endl;`;
            if (language === 'java') return node.isString ? `System.out.println("${node.value}");` : `System.out.println(${node.value});`;
            if (language === 'c') return node.isString ? `printf("${node.value}\\n");` : `printf("%d\\n", ${node.value});`;
            if (language === 'python') return node.isString ? `print("${node.value}")` : `print(${node.value})`;
